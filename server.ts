...
async function startServer() {
  ...
  app.post('/api/images', upload.single('image'), async (req, res) => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from('as_per_unit').getPublicUrl(filePath);

      const info = db
        .prepare('INSERT INTO images (url, category, alt) VALUES (?, ?, ?)')
        .run(publicUrl, category || 'general', alt || '');
  ...
  app.put('/api/images/:id', upload.single('image'), async (req, res) => {
    ...
    db.prepare(
      'UPDATE images SET url = ?, category = ?, alt = ? WHERE id = ?',
    ).run(
      url,
      category || existingImage.category,
      alt !== undefined ? alt : existingImage.alt,
      id,
    );
